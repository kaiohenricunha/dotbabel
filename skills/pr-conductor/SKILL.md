...
| --- | ---------------- | -------------------------------- | ------------------------------------------------------------ |
| 1   | `pre-pr`         | `commands/pre-pr.md`             | simplify, security-review, full test suite                   |
...
It is dry-run by default in an interactive session. Posting for real needs `--auto --confirm-post`. Under `--dry-run`, stop at the preview and report the comment count.
...
If the repo has no `.local-attest` config, skip this phase and say so plainly; CI will run remotely as normal.
...
PR #<N> — <title>   (base: <base>)
  ...
  3 post-pr-review  ✓ <k> comments posted
  4 review-pr       ✓ <k> resolved · pushed <sha> [skip ci]
  5 local-attest    ✓ attested <sha>   | SKIP no config | ✗ <reason>
