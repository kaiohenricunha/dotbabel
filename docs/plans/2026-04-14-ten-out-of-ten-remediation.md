# Plan — Raise `@kaiohenricunha/harness` (dotclaude) from 6.5/10 to 10/10

## Context

The repo at `/mnt/storage/Projects/kaiohenricunha/dotclaude` is graded **6.5/10 — Passable** by `docs/assessments/claude-config-management-2026-04-14.md`. It is a dual-purpose repository: a portable npm package + Claude Code plugin (`@kaiohenricunha/harness`) that bootstraps spec-driven-development governance into consumer repos, *and* Kaio's personal global Claude Code dotfiles symlinked into `~/.claude/` via `bootstrap.sh`. The code layer is solid (quality 7.8, tests 7.2, security 7.2) but productization is weak: `package.json:6` `"main"` points at a non-existent `plugins/harness/src/index.mjs`; both READMEs advertise a broken Node API import with *different* wrong paths; there is no `.github/workflows/` at root so the library publishes CI templates to consumers but runs zero CI on itself; `LICENSE`, `CHANGELOG`, `CONTRIBUTING`, `SECURITY.md` are missing; hardening decisions are crammed into a README table with no ADR context; `commands/*.md` lack the YAML frontmatter that `skills/*/SKILL.md` already uses; the repo ships a contract it does not itself follow (no root `docs/repo-facts.json`, no `.claude/skills-manifest.json`); every CLI's exit codes are inconsistent and no CLI supports `--json` / `--verbose` / `--help` / `--version`; the shipped `plugins/harness/templates/workflows/detect-drift.yml:15` invokes a non-existent `npx @kaiohenricunha/harness detect-drift` subcommand.
...
│
├─ .claude/                  (NEW — dogfood)
│  ├─ skills-manifest.json   (indexes commands/*.md + skills/*/SKILL.md + plugins/harness/commands/init-harness.md)
│  ├─ settings.json          (minimal: wires guard-destructive-git hook)
│  └─ hooks/guard-destructive-git.sh   (symlink to plugins/harness/hooks/)
│
...
│  ├─ specs/
│  │  ├─ README.md
│  │  └─ harness-core/{spec.json, spec.md}   (status: done; linked_paths cover the full plugin)
│  ├─ adr/
...
│
├─ examples/minimal-consumer/   (NEW — committed post-harness-init scaffold; source for dogfood.yml)
│
...
└─ plugins/harness/
   ...
   │  │  └─ debug.mjs        HARNESS_DEBUG=1 gate (replaces silent catches)
   ...
   │  ├─ harness-doctor.mjs               (NEW — env + manifest + specs + facts + drift + hook check)
   │  ├─ harness-detect-drift.mjs         (NEW — wraps scripts/detect-branch-drift.mjs; fixes detect-drift.yml)
   ...
   │  ├─ workflows/detect-drift.yml       (EDIT: call `npx harness-detect-drift`; Node 20+22 matrix)
   ...
   └─ tests/
      ...
      ├─ errors.test.mjs  output.test.mjs  argv.test.mjs  harness-doctor.test.mjs  index.test.mjs   (NEW)
...
**PR 1 — Foundation libs.** Create `plugins/harness/src/lib/{errors,exit-codes,output,argv,debug}.mjs` + unit tests (`errors.test.mjs`, `output.test.mjs`, `argv.test.mjs`). No behavior change yet. Establishes the `StructuredError` taxonomy (codes like `SPEC_STATUS_INVALID`, `MANIFEST_CHECKSUM_MISMATCH`, `COVERAGE_UNCOVERED`, `DRIFT_TEAM_COUNT`, `SETTINGS_SEC_1..4`) and the named exit-code enum. `output.mjs` mirrors the ✓/✗/⚠ format at `plugins/harness/scripts/validate-settings.sh:43-45` — that file is the house gold standard; reuse it verbatim.

**PR 2 — Barrel + umbrella CLI + package contract fixes.** Create `plugins/harness/src/index.mjs` re-exporting the 24-symbol public API (18 from `spec-harness-lib.mjs` + `validateSpecs`, `validateManifest`, `refreshChecksums`, `checkSpecCoverage`, `checkInstructionDrift`, `scaffoldHarness`, plus `version` / `ValidationError` / `EXIT_CODES`). Add `plugins/harness/bin/{harness, harness-doctor, harness-detect-drift}.mjs`. Edit `package.json`: valid `"main"`, new `"exports"` field (`"."`, `"./errors"`, `"./exit-codes"`, `"./package.json"`), add three new `"bin"` entries, add `"homepage"`, `"bugs"`, `"keywords"`, bump `"version": "0.2.0"`, add `"scripts"` (`lint`, `shellcheck`, `coverage`, `docs:links`, `dogfood`, `format:check`). Add `plugins/harness/scripts/` to `"files"` so `refresh-worktrees.sh` + `detect-branch-drift.mjs` + `auto-update-manifest.mjs` actually ship (currently advertised at `README.md:77-80` but excluded from `package.json:18-25`). Edit `plugins/harness/templates/workflows/detect-drift.yml:15` to `npx harness-detect-drift`. Edit both READMEs' import blocks (`README.md:67-72` and `plugins/harness/README.md:38-39`) to the single barrel form. Add `index.test.mjs` asserting 24 named exports.

**PR 3 — Adopt structured errors across validators.** Edit all six `src/*.mjs` modules to push `StructuredError` objects rather than strings. Every current error line gets a `code`, `file`, optional `pointer` / `line`, `expected`, `got`, `hint`, `category`. The enumerated rewrites: `validate-specs.mjs:47,55,61,66,71,76,85,87,96,98,103,108,125`; `validate-skills-inventory.mjs:49,54,61,73`; `check-instruction-drift.mjs:32,42,53,59,74,90`; `check-spec-coverage.mjs:43,50,55`; `init-harness-scaffold.mjs` conflict + usage messages. Update the six existing test files to assert on `.code` + `.message` (backwards-compat `toString()` on `ValidationError`). Add JSDoc `@param`/`@returns`/`@typedef HarnessContext` / `@typedef ValidationResult` to every export in `spec-harness-lib.mjs`. Replace the silent catches at `spec-harness-lib.mjs:28-29` (`resolveRepoRootFromGit`) and `:184-186` (`getChangedFiles`) with `debug("git:rev-parse", err.message)` etc. Rewrite every bin to use `lib/{argv, output, errors, exit-codes}` and add `--help`, `--version`, `--json`, `--verbose`, `--no-color`. Normalize exits per the convention (0/1/2/64). `harness-init.mjs:25,33,62` switch to the named constants.

**PR 4 — Shell upgrades + bats suite.** Change `plugins/harness/scripts/validate-settings.sh:26` from `set -u` to `set -euo pipefail`. Factor the ✓/✗/⚠ helpers at `:43-45` into `plugins/harness/scripts/lib/output.sh` and `source` them; same file adds a `--json` mode emitting `{check, category, status, message}`. Harden `plugins/harness/hooks/guard-destructive-git.sh:16`: normalize tabs → single space, match on `(^|[[:space:]]|[;&|])git[[:space:]]+` plus each destructive verb; add `BYPASS_DESTRUCTIVE_GIT=1` env bypass; rewrite block message to show the bypass form. **Preserve the existing `exit 2` semantics** — that is Claude Code's hook protocol (exit 2 blocks the tool call), not the validator exit convention. Mirror the hardening into `plugins/harness/templates/claude/hooks/guard-destructive-git.sh`. Edit `bootstrap.sh` to add a `--quiet` flag and a trailing `"run harness doctor to verify install"` line when `harness-doctor` is on PATH. Edit `sync.sh:15` to run a secret-scan before `git add -A`. Add bats-core suite under `plugins/harness/tests/bats/`: `bootstrap.bats` (first-run, idempotency, real-file backup, broken symlink repair, stale target update), `sync.bats` (pull/push/status with fake-git shim; secret-scan abort), `refresh-worktrees.bats` (dirty skip, FF-merge, conflict reporting), `guard-destructive-git.bats` (every blocked pattern, tab/space variants, bypass env, harmless `git reset --soft` allowed). Add `.shellcheckrc`, `.prettierrc.json`, `.editorconfig`, `.markdownlint-cli2.jsonc`. Extend `plugins/harness/tests/test_validate_settings.sh` from 8 to 12 cases.

**PR 5 — Dogfood + integration + missing script tests.** Create root `.claude/skills-manifest.json` inventorying every file under `commands/`, `skills/*/SKILL.md`, and `plugins/harness/commands/init-harness.md`; `.claude/settings.json` wiring the guard hook; `.claude/hooks/guard-destructive-git.sh` symlinked to `plugins/harness/hooks/`. Create root `docs/repo-facts.json` with `team_count: 1`, protected_paths covering `CLAUDE.md`, `README.md`, `.github/workflows/**`, `.claude/**`, `docs/repo-facts.json`, `docs/specs/**/spec.json`, `plugins/harness/src/**`, `plugins/harness/bin/**`, `plugins/harness/templates/**`. Create `docs/specs/harness-core/{spec.json,spec.md}` with `status: done`, full `linked_paths` and `acceptance_commands: ["npm test", "npx harness-doctor", "node -e 'import(\"@kaiohenricunha/harness\").then(m => process.exit(Object.keys(m).length >= 24 ? 0 : 1))'"]`. Create `docs/specs/README.md` framed for this repo (reuse the template at `plugins/harness/templates/docs/specs/README.md`). Add the missing `plugins/harness/tests/{detect-branch-drift,auto-update-manifest}.test.mjs`. Add `plugins/harness/tests/integration/end-to-end-scaffold.test.mjs` that `git init`s a tmpdir, runs `harness-init`, then runs each validator sequentially and asserts exit 0. Create `examples/minimal-consumer/` by running `harness-init` into it and committing the output. Add `vitest.config.mjs` with `coverage.thresholds: {lines: 85, functions: 85, branches: 80, statements: 85}`.

**PR 6 — Docs workstream (the priority).** Create in this order so later files cite earlier ones:
- `LICENSE` (MIT full text).
- `CHANGELOG.md` — Keep-a-Changelog; `[0.1.0] - 2026-04-13` retroactive; `[0.2.0] - <ship date>` enumerating every change from this plan (barrel export, structured errors, named exit codes, `--json`/`--verbose`/`--help`/`--version`/`--no-color`, `harness doctor`, `harness-detect-drift`, umbrella `harness` bin, bats suite, full CI, Dependabot, CodeQL, release automation with provenance, ADRs, LICENSE, all docs); call out as breaking for consumers parsing validator stderr.
- `CONTRIBUTING.md` — clone → bootstrap → test (`npm test && bash plugins/harness/tests/test_validate_settings.sh && bats plugins/harness/tests/bats/`); spec-before-code discipline pointing at `docs/specs/harness-core/`; commit + PR conventions; worktree discipline lifted from current `CLAUDE.md:48-52`.
...
- `plugins/harness/templates/README.md` — placeholder catalog (`{{project_name}}`, `{{project_type}}`, `{{today}}` cite `init-harness-scaffold.mjs:21-27` + `init-harness-scaffold.test.mjs:116-129`) + per-template rationale.
...
- `plugins/harness/README.md` — slimmed to npm-tarball-only, ≤80 lines, pointer back to root docs/.
...
**PR 7 — CI + release.** Create `.github/CODEOWNERS`, `.github/dependabot.yml` (weekly npm + github-actions ecosystems; inspect `plugins/harness/templates/workflows/` too so consumers get SHA refreshes), `.github/PULL_REQUEST_TEMPLATE.md` (enforces Spec ID / `## No-spec rationale` per `check-spec-coverage.mjs:42-44`), `.github/ISSUE_TEMPLATE/{bug_report.yml, feature_request.yml, config.yml}`. Add workflows in dependency order:
- `test.yml` — matrix `{node: [20, 22]}` × ubuntu-latest; `npm ci`, `npm test -- --coverage`, `bash plugins/harness/tests/test_validate_settings.sh`, `bats plugins/harness/tests/bats/`; upload coverage artifact.
...
- `docs.yml` — on PRs touching `plugins/harness/src/**.mjs`: regenerate `docs/api-reference.md` via `scripts/generate-api-reference.mjs`; fail PR if disk ≠ regenerated.
...
Pin every action by commit SHA, mirroring `plugins/harness/templates/workflows/validate-skills.yml:19-20`. After all workflows are green on a dry-run PR, finalize CHANGELOG, tag `v0.2.0`, push → `release.yml` publishes.

## Critical files

- `plugins/harness/src/index.mjs` (NEW — barrel of 24 public symbols; fixes `package.json:6`)
- `plugins/harness/src/lib/errors.mjs` (NEW — StructuredError taxonomy; adopted by all six validators)
- `plugins/harness/src/lib/output.mjs` (NEW — mirrors `plugins/harness/scripts/validate-settings.sh:43-45` ✓/✗/⚠ format with `--json` + `--no-color`)
- `plugins/harness/src/lib/exit-codes.mjs` (NEW — `{OK:0, VALIDATION:1, ENV:2, USAGE:64}`)
- `plugins/harness/src/spec-harness-lib.mjs` (EDIT — JSDoc all 18 exports; replace silent catches at :28-29 + :184-186 with `debug()`)
- `plugins/harness/bin/harness.mjs`, `harness-doctor.mjs`, `harness-detect-drift.mjs` (NEW bins)
- `package.json` (EDIT — main, exports, bin × 3, scripts × 6, files += scripts/, version bump)
- `plugins/harness/scripts/validate-settings.sh` (EDIT — `set -u` → `set -euo pipefail` at :26; source shared `lib/output.sh`)
- `plugins/harness/hooks/guard-destructive-git.sh` (EDIT — harden regex at :16; add bypass; **preserve `exit 2` per Claude Code hook protocol**)
- `plugins/harness/templates/workflows/detect-drift.yml` (EDIT — fix :15 to `npx harness-detect-drift`)
- `README.md` (EDIT — full rewrite; fix broken import block at :67-72; delete dead-link line at :129)
- `plugins/harness/README.md` (EDIT — slim; single barrel import)
- `CLAUDE.md` (EDIT — persona header; relocate contributor rules)

## Reuse map (existing helpers to build on — do not reinvent)

- **`createHarnessContext` at `plugins/harness/src/spec-harness-lib.mjs:5-21`** — every new bin (`harness-doctor`, umbrella `harness`) resolves repo root through this, preserving the three-step fallback (arg → `HARNESS_REPO_ROOT` → `git rev-parse`).
- **`readJson` / `readText` / `pathExists` / `git` / `loadFacts` / `listSpecDirs` / `listRepoPaths` / `toPosix`** (`spec-harness-lib.mjs:33-98`) — `harness-doctor` and the end-to-end integration test reuse these verbatim; no new filesystem helpers.
...
- **PREFIX_MAP + substitutePlaceholders** at `plugins/harness/src/init-harness-scaffold.mjs:5-9,21-27` — the `plugins/harness/templates/README.md` placeholder catalog cites these as the canonical substitution site; no behavior change to the scaffolder itself.
- **Gold-standard shell UI** at `plugins/harness/scripts/validate-settings.sh:43-45` — `plugins/harness/src/lib/output.mjs` and `plugins/harness/scripts/lib/output.sh` both reproduce the exact `pass()`/`fail()`/`warn()` prefix format (✓/✗/⚠ + ANSI gate on `-t 1`).
- **SHA-pinning convention** at `plugins/harness/templates/workflows/validate-skills.yml:19-20` and `ai-review.yml:21` — every new `.github/workflows/*.yml` follows the same commit-SHA pinning.
- **Fixture helper pattern** at `plugins/harness/tests/validate-specs.test.mjs:12-16` (`mkdtempSync` + `cpSync`) — all new hermetic tests use the same.
- **Existing JSDoc house style** at `plugins/harness/src/validate-specs.mjs:17-33` + `check-instruction-drift.mjs:7-24` — the JSDoc rollout across `spec-harness-lib.mjs` matches this voice and `@param`/`@returns` shape.
...
- **Architecture (target ≥9).** `node -e 'import("@kaiohenricunha/harness").then(m => process.exit(Object.keys(m).length >= 24 ? 0 : 1))'` exits 0. `grep -R "plugins/harness/src" README.md plugins/harness/README.md` returns nothing. `npx harness-detect-drift --help` prints flags. Dogfood: root `npx harness-validate-skills && npx harness-validate-specs && npx harness-check-instruction-drift && npx harness-check-spec-coverage` all exit 0.
- **Code quality (≥9).** `npm run lint` (prettier) green. `shellcheck -x bootstrap.sh sync.sh plugins/harness/scripts/*.sh plugins/harness/hooks/*.sh plugins/harness/tests/*.sh plugins/harness/templates/claude/hooks/*.sh plugins/harness/templates/githooks/pre-commit` green. `node scripts/check-jsdoc-coverage.mjs plugins/harness/src` green. `markdownlint-cli2 "**/*.md" "#node_modules"` green.
- **Test coverage (≥9).** `npm test -- --coverage` meets all four thresholds. `bats plugins/harness/tests/bats/` all pass. `bash plugins/harness/tests/test_validate_settings.sh` reports 12/12. `plugins/harness/tests/integration/end-to-end-scaffold.test.mjs` green. `test.yml` green on Node 20 + 22.
- **CI/CD (≥9).** All seven workflows green on a representative PR. `dogfood.yml` green against both root and `examples/minimal-consumer/`. `release.yml` successfully publishes `v0.2.0` with `npm view @kaiohenricunha/harness@0.2.0` showing provenance. First Dependabot PR arrives within a week.
...
- **Observability (≥9).** Every bin's `--help`, `--version`, `--json`, `--verbose`, `--no-color` works; exit codes follow `{0,1,2,64}` (asserted in integration test). `npx harness doctor` runs in `dogfood.yml` and reports every check green. `bats guard-destructive-git.bats` asserts block message contains `BYPASS_DESTRUCTIVE_GIT=1`. `validate-settings.sh --json` returns valid JSON (asserted in `test_validate_settings.sh` case #12).
...
- TypeScript migration — captured in ADR-0002; revisit at v0.3 if `.d.ts` consumer demand surfaces (mitigation: hand-write `plugins/harness/src/index.d.ts` later).
