...
| R-2 | `npm update -g` in `sync pull` updates ALL globally installed packages, not just `@dotbabel/dotbabel`                                                 | Low        | Low    | Use `npm update -g @dotbabel/dotbabel` (package-scoped) not `npm update -g` bare.                                                                                           |
...
| R-4 | `bootstrap.sh` and `dotbabel bootstrap` diverge over time — someone updates one but not the other                                                     | Medium     | Medium | Add a note in `bootstrap.sh` referencing the CLI equivalent. The integration test for bootstrap-global.mjs runs against the same expected outputs as the bootstrap.sh test. |
...
A-1: **Ship only a wrapper that calls `bootstrap.sh`** — have `dotbabel bootstrap`
...
A-2: **Clone dotbabel from GitHub if no local clone is detected** — have
`dotbabel bootstrap` run `git clone` in npm mode to create a local clone in
`~/.dotbabel/` and then symlink from there. Rejected because it introduces
...
A-4: **Use a `dotbabel setup` name instead of `dotbabel bootstrap`** —
...
reduces confusion; new devs who know `./bootstrap.sh` will find `dotbabel bootstrap`
immediately intuitive.

A-5: **Merge `sync` into `bootstrap` as flags** — `dotbabel bootstrap --pull`
