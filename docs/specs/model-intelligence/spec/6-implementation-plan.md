...
- **IMPL-19**: When a Model Intelligence pull request changes a file that another spec's `linked_paths` also covers, the `## Spec ID` section names both spec ids, and `dotbabel criteria verify --pr <N> --post` runs before the merge gate. Most implementation PRs meet this condition: `qa-verification-harness` links `plugins/dotbabel/src/quality/**`, `plugins/dotbabel/bin/dotbabel.mjs`, and `schemas/dotbabel.config.schema.json`, all of which this spec's units touch. The reason the declaration is load-bearing is an asymmetry between the two readers, and it is not the obvious one. The merge gate scopes criteria to the **union** of the ids the body declares and the ids the diff implicates through `linked_paths` at the base ref (`plugins/dotbabel/src/criteria/gate-inputs.mjs:131,146`), so a pull request cannot escape a spec by staying silent about it — the gate will require that spec's active criteria either way. `criteria verify --pr <N>` reads the body alone (`plugins/dotbabel/src/criteria/preconditions.mjs:60`). An undeclared spec is therefore required by the gate and skipped by the verifier, which reports success while leaving those criteria without evidence. Declaring both ids is what makes the verifier produce the evidence the gate already demands. PR #393 failed exactly this way.
