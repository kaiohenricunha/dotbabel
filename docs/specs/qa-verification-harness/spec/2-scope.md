...
1. **Acceptance criteria in `spec.json`.** Each entry carries an id, a `planned` or `active` status, `given`, `when`, and `then` statements, the tests that prove it, one command that runs them, and an optional test report, as KD-1, KD-3, and KD-15 in §4 and the schema in §5 set. The field is optional. `dotbabel-validate-specs` checks its shape only in a spec that declares it, so every existing spec stays valid.
...
3. **Criteria verification in `review-pr`.** A new dotbabel command runs the tests that each criterion names, and it records the test name, the run output, and the criterion id as evidence pinned to the head commit. `review-pr` calls it as its last step, after the final branch health gate, in standalone and conductor mode alike. The evidence therefore matches the head that merges, and a failing criterion stops the conductor before the `local-attest` matrix. The step runs only when a linked spec declares an active criterion. A new reason code in `pr-gates.mjs` holds the merge gate when the evidence is missing, failing, or pinned to an older commit.
...
| `plugins/dotbabel/src/local-attest-lib.mjs` and `plugins/dotbabel/src/lib/attest-marker.mjs`: shared marker helpers                  | The local-attest marker text that `.github/workflows/test.yml` reads                                                    |
