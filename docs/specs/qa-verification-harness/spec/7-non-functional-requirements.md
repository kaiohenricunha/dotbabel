...
- **REL-25**: A pull request that edits a governed file has a defined disposition, never a dead end. The merge gate reports `attestation.state: explicit` with an `ATTESTATION_GOVERNED_CHANGE` warning that names the files, and adds no blocking attestation reason. The first design reported `ATTESTATION_CONFIG_CHANGED` for this case and told `/merge-pr` to stop; nothing routed forward, and re-running `/local-attest` cannot clear it, so the first Dependabot bump of `package.json` would have been unmergeable through the sanctioned path. `ATTESTATION_CONFIG_CHANGED` now means only that the base moved under the pull request, whose recovery is a rebase.
...
- **SEC-17**: On the explicit path for a governed-file pull request, `/merge-pr` shows the user the governed-file diff and continues only on their explicit acknowledgement, before it runs anything. That path executes the pull request's own scripts, so a change that rewrote its `test` script to `true` would pass its own verification; the human review of that diff is the one control that cannot be edited away, and a green suite is not that acknowledgement.
