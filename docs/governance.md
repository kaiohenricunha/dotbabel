...
3. Rebuild the index: `node plugins/dotbabel/bin/dotbabel-index.mjs`.
...
1. Verify zero artifacts use the value (`dotbabel list --domain <value>` returns nothing).
...
1. Frontmatter validates against the type's schema (`dotbabel-validate-skills`).
2. `id` == filename basename (enforced by schema).
3. All `related` and `deprecated_by` ids resolve in the index.
4. Index rebuilds cleanly: `dotbabel-index --check` exits 0.
...
Once per quarter, run `dotbabel list --json | jq 'group_by(.facets.domain[]) | ...'` to produce a facet usage report. Any value used by fewer than 3 artifacts after 6 months is a candidate for removal or merger.
