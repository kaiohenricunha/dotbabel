...
| Running several sessions in one repository   | [fleet.md](./fleet.md) — file claims, the deny-on-conflict hook, and its limits    |
...
- [specs/dotbabel-core/](./specs/dotbabel-core/) — the canonical spec this repo governs itself with
...
- **Umbrella CLI + standalone bins.** `dotbabel validate-specs` or
  `dotbabel-validate-specs` — both exist, same behavior.
