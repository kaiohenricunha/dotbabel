# Copilot PR review instructions for `@dotbabel/dotbabel`
...
- **Protected paths without spec coverage.** If the PR touches any path in
  ...
  `Spec ID: dotbabel-core`) or `## No-spec rationale`. Flag if missing —
  this is what `dotbabel-check-spec-coverage` enforces and a missing
  block fails CI.
...
- **Drift between `CLAUDE.md` "Protected paths" and `docs/repo-facts.json`.**
  ...
  vice versa — `dotbabel-check-instruction-drift` enforces this.
...
### CLI / library contract (`plugins/dotbabel/`)
...
- **Bin without the standard pipeline.** Validator-style
  `plugins/dotbabel/bin/*.mjs` must use
...
formatError(lib/errors) → exit(lib/exit-codes)`. Flag raw
  ...
  validator bins. Exception: `dotbabel.mjs` (umbrella dispatcher) and
  `dotbabel-detect-drift.mjs` (thin wrapper) intentionally use
  `spawn` / `process.exit`.
...
- **Missing JSDoc on exports.** `scripts/check-jsdoc-coverage.mjs` fails
  CI on undocumented `export`s under `plugins/dotbabel/src/`. Flag any
...
- **Cross-plugin import.** `plugins/dotbabel/` and `plugins/harness/`
...
- Files under `commands/`, `skills/`, `schemas/`, `CLAUDE.md`,
  `plugins/dotbabel/{src,bin,scripts,templates,hooks}/`,
  `plugins/dotbabel/README.md`, and
  `plugins/dotbabel/.claude-plugin/` ship in the npm tarball
...
- Agent template under `plugins/dotbabel/templates/claude/agents/`
...
- New code path in `plugins/dotbabel/src/` without a corresponding
  `plugins/dotbabel/tests/*.test.mjs` covering it. Coverage thresholds
