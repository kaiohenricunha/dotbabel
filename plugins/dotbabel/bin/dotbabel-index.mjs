#!/usr/bin/env node
/**
 * dotbabel-index — rebuilds the taxonomy index under `<repo>/index/`.
 *
...
const META = {
  name: "dotbabel-index",
  synopsis: "dotbabel-index [OPTIONS]",
  description:
...
if (argv.flags.check) {
  ...
  if (stale) {
    out.fail(
      "index is stale — run `dotbabel-index` to refresh index/artifacts.json, index/by-type.json, index/by-facet.json",
    );
    out.flush();
    process.exit(EXIT_CODES.VALIDATION);
  }
...
if (!existsSync(readmePath)) {
  writeFileSync(
    ...
    [
      "# index/",
      "",
      "Generated by `dotbabel-index`. Do not hand-edit `artifacts.json`, `by-type.json`, or `by-facet.json` — they are rebuilt from authored artifacts under `agents/`, `skills/`, `commands/`, `hooks/`, and `templates/`.",
      "",
      "To regenerate:",
      "",
      "```bash",
