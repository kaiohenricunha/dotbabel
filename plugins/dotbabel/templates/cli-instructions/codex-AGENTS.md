...
# Global Codex CLI rules


Universal behavior for every Codex CLI session in every repo. Project-level `CLAUDE.md` files extend and may override these, but should not repeat them.


## Local filesystem conventions

- All projects live at `$HOME/projects/`. Do not search the home directory or default locations.
- Global Codex config lives wherever you cloned `dotbabel` and is symlinked into `~/.codex/`. Edit files in the clone, not `~/.codex/` directly.
