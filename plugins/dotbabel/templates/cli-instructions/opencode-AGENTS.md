...
# Global OpenCode CLI rules

Universal behavior for every OpenCode CLI session in every repo. Project-level `CLAUDE.md` files extend and may override these, but should not repeat them.

## Local filesystem conventions

- All projects live at `$HOME/projects/`. Do not search the home directory or default locations.
- Global OpenCode config lives wherever you cloned `dotbabel` and is symlinked into `~/.config/opencode/`. Edit files in the clone, not `~/.config/opencode/` directly.
