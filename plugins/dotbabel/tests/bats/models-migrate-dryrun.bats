...
@test "models migrate --json: stdout is one parseable document and stays write-free" {
  ...
  [ "$status" -eq 0 ]
  echo "$output" | node -e 'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>{const r=JSON.parse(s);if(r.mode!=="analysis")throw new Error("mode");if(r.totals.mapped!==1)throw new Error("totals");})'
